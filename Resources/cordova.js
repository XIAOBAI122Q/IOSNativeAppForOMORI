    ...
    addConstructor: function (func) {
        channel.onCordovaReady.subscribe(function () {
            try {
                ...
                // 在iOS上禁用调试日志
                if (!(window.navigator && window.navigator.userAgent && window.navigator.userAgent.indexOf('iPhone') != -1)) {
...
module.exports = {
    exec: function (bridgeSecret, service, action, callbackId, argsJson) {
        // 直接禁用所有prompt调用
        console.log('Cordova: BLOCKED exec prompt');
        return null;
    },
    setNativeToJsBridgeMode: function (bridgeSecret, value) {
        // 直接禁用所有prompt调用
        console.log('Cordova: BLOCKED bridge_mode prompt');
        return;
    },
    retrieveJsMessages: function (bridgeSecret, fromOnlineEvent) {
        // 直接禁用所有prompt调用
        console.log('Cordova: BLOCKED poll prompt');
        return '[]';
    }
};
...
function androidExec (success, fail, service, action, args) {
    ...
    // On iOS, silently return without executing Android-specific code
    if (isIOS) {
        // Call fail callback if provided to indicate the feature is not available
        if (fail) {
            setTimeout(function() {
                fail({ code: 'NOT_AVAILABLE', message: 'Android-specific feature not available on iOS' });
            }, 0);
        }
        return;
    }
...
androidExec.init = function () {
    // 直接禁用gap_init弹窗，不做任何设备检测
    bridgeSecret = Math.random();
    console.log('Cordova: gap_init prompt BLOCKED');
